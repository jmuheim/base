...
This is a basic Rails project with everything configured the way I want. {Fork it}[http://bitdrift.com/post/4534738938/fork-your-own-project-on-github] to create a new project out of it.

In your fork:

- Create a <code>README.md</code> file with the contents of <code>README_example.md</code>, then...
...
- Change the Rack Livereload port in <code>Guardfile</code> and `config/development.rb`
- Change the default URL options' <code>:host</code> in <code>config/environments/production.rb</code> to <code>http://ACCOUNT.SERVER.uberspace.de</code>
- Copy <code>config/deploy_example.rb</code> to <code>config/deploy.rb</code> and replace <code>ACCOUNT</code>, <code>PROJECT</code> and <code>SERVER</code> with your project specific data.